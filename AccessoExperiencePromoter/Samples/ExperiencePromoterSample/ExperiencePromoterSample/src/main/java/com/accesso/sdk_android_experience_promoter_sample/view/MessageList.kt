 ...
 */
@Composable
fun MessagesList(navController: NavHostController, viewModel: MessagesViewModel) {

    val messages = viewModel.messagesLiveData.observeAsState()

    messages.value.let { messageList ->
        if (messageList.isNullOrEmpty()) {
            Column {
                Row(modifier = Modifier
                    ...
                    Text(text = "No Messages", color = Color.Black.copy(alpha = 0.5f))
                }
                Row(modifier = Modifier
                    .fillMaxWidth(),
                    horizontalArrangement = Arrangement.Center
        ...
        LazyColumn(
            ...
            if (messageList != null) {
                items(messageList) { message ->
                    if (message != null) {
                        MessageListItem(message = message, navController, viewModel)
                    }
                }
            }
        }
